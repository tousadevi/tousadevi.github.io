<?php
include 'config.php';
$sql = "select * from registration where status = 'suspended'";
...
<script src="jscripts/requests.js"> </script>
 <body>
 <form  id = "suspenduser" method="Post" name = "suspenduser">
 <div id="banner"><img src="suspendedusers.jpg" width="100%" height="10%" alt="banner" />
</div>
 <table align = "center">
  <tr>
    <th>Firstname</th>
...
$numb = 0;
  while($registration = $records->fetch_assoc())
	...
	<td><input style="border:none;border-color:transparent;"  type="hidden" size="15" name="password" id="password<?php echo $numb ?>" value="<?php echo $registration['password'];?>" readonly/></td>  
     ...
     <td><button type = "button" onclick = "aceeptRecord(<?php echo $numb ?>)" class="button">Re-activate</button></td>
	 <td><button type = "button" onclick = "deleteuser(<?php echo $numb ?>)" class="button">Delete</button></td>
	<?php
	 echo "</tr>";
	 $numb++;
  }
